...
# period = 2
# frecuency = 'day'
print(f"Target coords: {target_coords} Period: {period} Frecuency: {frecuency}")
...
for rcm_name, data_rcm in utils.data_rcm_paths.items():
    ...
    if period == 1:
        ...
        data_climatology = {season: {rcm_name : {gcm_name: gcm_name}} for season in seasons}
        utils.mem_status("Despues de seleccionar pr hist")
        for season in seasons:
            data_yearly = utils.get_season_data(data_selected, season)

            data_max = data_yearly.max(dim="time")
            time_dim = 'year' if season != 'DJF' else 'season_year'
            data_max_max = data_max.max(dim=time_dim)*time_multiplier

            data_climatology[season] = data_max_max# - data_prh_climatology[season][rcm_name][gcm_name]
        utils.mem_status("Despues de calcular max hist")
        ds_combined = xr.concat(
            [data_climatology[season] for season in seasons],
                dim='season'
            )
        utils.mem_status("Despues de concatenar hist")
        ds_combined = ds_combined.assign_coords(season=seasons)
        ds_combined.to_netcdf(f"{DATA_PATH_METRICS}{folder_name}/climatology_max_{frecuency}_{rcm_name}_{target_coords}_1995-2015_mask.nc", compute=True)
        utils.mem_status("Despues de guardar hist")
    ...
    if period == 2:
        ...
        for season in seasons:
            ...
            data_future[season] = data_max_max #- data_climatology[season][rcm_name][data_rcm['gcm']]
        ...
        ds_combined = ds_combined.assign_coords(season=seasons)
        ds_combined.to_netcdf(f"{DATA_PATH_METRICS}{folder_name}/ssp370_max_{frecuency}_{rcm_name}_{target_coords}_CWL3_mask.nc", compute=True)
